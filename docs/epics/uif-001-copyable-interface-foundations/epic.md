...
The system SHALL provide a text-first SegmentedControl, EditorToolbar, DocumentHeader, and EditorSurface composition for trailing or deliberately centered Source/Rendered modes, semantic commands, title-initiated controlled inline filename editing with a preserved final extension, operation and recovery states, stable editor-region slots, and application-owned behavior.
...
- AND a consumer may compose the exported mode switch, routine status, and labeled overflow action in `DocumentHeader.trailingActions` on the document-identity row without an opaque mode-switch container.
...
- THEN the labeled name input receives focus and remains editable while any final filename extension is visible but not editable
- AND Enter submits the rename form, Escape cancels, and visible Save name and Cancel actions delegate the full filename with its original final extension without owning rename validation or persistence.
...
| S4/R2-S1 | `src/components/SegmentedControl/SegmentedControl.tsx#SegmentedControl`; `src/patterns/EditorToolbar/EditorToolbar.tsx#EditorToolbar`; `src/patterns/EditorToolbar/EditorToolbar.tsx#EditorModeSwitch` | primary | Own text-first controlled single-selection, canonical trailing mode placement, explicit center placement, transparent work-surface switch composition, and Source/Rendered delegation. |
| S4/R2-S1 | `src/patterns/WorkbenchPatterns.stories.tsx#DocumentEditingReference` | support | Composes mode, autosave status, and labeled overflow action in the document header for the Markdown editing reference. |
...
| S4/R2-S1, S4/R2-S2, S4/R2-S3, S4/R2-S4 | Browser inspection of Document Editing and Editor Recovery at `1440x900` and `390x844` | Title trigger, immutable extension, keyboard rename controls, transparent header-aligned mode/status/overflow group, recovery states, long filenames, and editor alignment reflow without overlap or horizontal overflow. | Passing 2026-07-24 |
