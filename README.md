...
`DocumentHeader` keeps document context and controlled inline renaming together: supplying `rename` makes the visible title the accessible edit trigger, presents the final filename extension as a fixed suffix outside the editable name, and preserves that suffix in consumer callbacks; the application retains draft, validation, persistence, and permission policy. Its trailing-actions slot can compose the exported Source/Rendered switch, routine autosave status, and a labeled overflow action beside document identity. `EditorToolbar` accepts text-first command groups, a trailing Source/Rendered switch by default, one operation-status region, and trailing actions; consumers can opt into centered mode placement for deliberate legacy composition. Routine dirty, saving, saved, and read-only feedback belongs in `OperationStatus`; conflicts and failures that require a decision belong in persistent `InlineNotice` compositions with visible actions.
